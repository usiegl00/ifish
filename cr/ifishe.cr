...
def siv(k, p)
  ...
  bf.encrypt
  t = String.new s2v(k[0..15], p)
  bf.iv = t
  bf.key = k[16..31]
  return t + String.new(bf.update(p)) + String.new(bf.final)
end
...
puts Base64.encode siv (cmac("\x00"*16,k).to_a+cmac("\x20"*16,k).to_a+cmac("\x40"*16,k).to_a+cmac("\x60"*16,k).to_a+cmac("\x80"*16,k).to_a).to_s, ARGF.gets_to_end
