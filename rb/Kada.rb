...
require "openssl"
def dsv(k,e,s=[])
  bf = OpenSSL::Cipher.new("blowfish").decrypt
  k1 = k[0..15]
  k2 = k[16..31]
  bf.iv = e[0..7]
  bf.key = k2
  p = (bf.update(e[8...e.size]) + bf.final).rstrip.chomp("\x01")
  if s2v(k1,p,s) == e[0..7]
    return p
  else
    return false
...
puts dsv(tree(IO::console.getpass("KEY:")).unpack("H*")[0],ARGF.read.strip.unpack("m")[0])
